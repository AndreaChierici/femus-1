...
#include "TetCutNetEval.hpp"

#include <random>

#include <chrono>
#include <atomic>
static std::atomic<long long> _gnTetNanos{0};
static std::atomic<long> _gnTetCalls{0};
struct _GnTimerReport {
  ~_GnTimerReport() {
    std::cout << "[GN-TIMER] GetNormalTet: " << _gnTetCalls.load() << " calls, "
    << _gnTetNanos.load()/1e9 << " s total" << std::endl;
  }
};
static _GnTimerReport _gnTetReport;
...
// void GetNormalTetBF1(const std::vector < std::vector<double> > &xv, const std::vector<double> &xg, const double & R, std::vector<double> &a, double & d,  std::vector<double> &xm, std::vector<double> &a2, double & d2, double & volume,  unsigned & cut) {
  ...
  if(cnt == 0) {
    ...
    a2[1] /= bNorm;
    a2[2] /= bNorm;
    double d2Geo = - a2[0] * xi[0] - a2[1] * xi[1] - a2[2] * xi[2];

    // no_crossings feature set: [a2_0, a2_1, a2_2, t1, cnt, d2_geo]
    std::array<float, 6> features = {
      (float) a2[0], (float) a2[1], (float) a2[2],
      (float) t1, (float) cnt, (float) d2Geo
    };


    // // static long _dumpTotal = 0;
    // // static int _dumpCount = 0;
    // // static long _stride = getenv("DUMP_STRIDE") ? atol(getenv("DUMP_STRIDE")) : 997;
    // // if (_dumpCount < 5000 && _dumpTotal % _stride == 0) {
    // //   std::cout << "[DUMP] " << _dumpCount << " "
    // //   << xv[0][0] << " " << xv[1][0] << " " << xv[2][0] << " "
    // //   << xv[0][1] << " " << xv[1][1] << " " << xv[2][1] << " "
    // //   << xv[0][2] << " " << xv[1][2] << " " << xv[2][2] << " "
    // //   << xv[0][3] << " " << xv[1][3] << " " << xv[2][3] << " "
    // //   << xg[0] << " " << xg[1] << " " << xg[2] << " " << R
    // //   << std::endl;
    // //   _dumpCount++;
    // // }
    // // _dumpTotal++;
    // //
    // // if (_dumpTotal % 1000000 == 0) std::cout << "[CALLS] " << _dumpTotal << std::endl;

    // d2 = EvalTetCutNet(features);
    d2 = d2Geo;

    // static std::mt19937 _gen(42);
    // static std::normal_distribution<double> _noise(0.0, 0.01);   // then rerun with 0.001
    // d2 = d2Geo + _noise(_gen);

  }

  // _gnTetNanos += std::chrono::duration_cast<std::chrono::nanoseconds>(
