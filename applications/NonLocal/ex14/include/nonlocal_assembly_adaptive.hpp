...
void AssembleNonLocalRefined(MultiLevelProblem& ml_prob) {
  ...
  unsigned thetaIndex = mlSol->GetIndex("theta");
...
//   double eps = 0.125 * dMax;
  ...
  if(dim == 3) {
    ...
    refineElement[3][0] = new RefineElement(lmax1, "quad", "linear", "fifth", "ninth", "legendre");
    refineElement[3][1] = new RefineElement(lmax1, "quad", "quadratic", "fifth", "ninth", "legendre");
    refineElement[3][2] = new RefineElement(lmax1, "quad", "biquadratic", "fifth", "ninth", "legendre");

    refineElement[4][0] = new RefineElement(lmax1, "tri", "linear", "fifth", "ninth", "legendre");
    refineElement[4][1] = new RefineElement(lmax1, "tri", "quadratic", "fifth", "ninth", "legendre");
    refineElement[4][2] = new RefineElement(lmax1, "tri", "biquadratic", "fifth", "ninth", "legendre");

    refineElement[3][soluType]->SetConstants(eps);
    refineElement[4][soluType]->SetConstants(eps);

    // nonlocal = new NonLocalBall();
    nonlocal = new NonLocalBallFrac();
    //nonlocal = new NonLocalBox();
  }

  nonlocal->SetKernel(kappa1, delta1, eps);

  std::cout << "kernel = " << nonlocal->GetKernel() << std::endl;

  // regularized fractional consistency source for u = x^2 + y^2:
  //   f = L_reg u = -kappa * (delta - 3*d0/4)   for d0 <= delta
  // (equals -kappa*delta/4 at d0 = delta; -> -kappa*delta as d0 -> 0)
  const char *fd0env = getenv("FRAC_D0");
  const double d0read = fd0env ? atof(fd0env) : 1.0e10;
  const double d0eff = (d0read < delta1) ? d0read : delta1;
  const double sqD   = sqrt(delta1 * delta1 + d0eff * d0eff);
  // const double fFrac = -2. * kappa1 * (sqD + d0eff * d0eff / sqD - 2. * d0eff);
  const double fFrac = 1.;

  // const double d0eff = (d0read < delta1) ? d0read : delta1;
  // const double fFrac = -2. * kappa1 * (delta1 - 0.75 * d0eff);
  ...
  const bool tailOn = getenv("FRAC_TAIL") && atoi(getenv("FRAC_TAIL"));
  const double massCoeff = kappa1 / sqrt(delta1 * delta1 + d0eff * d0eff);
  std::vector< std::vector<double> >   tailJac;
  std::vector< std::vector<unsigned> > tailMap;
  ...
  for(unsigned iel = offset; iel < offsetp1; iel++) {
    ...
    for(unsigned i = 0; i < nDof1; i++) {
      ...
      solTheta1[i] = (*sol->_Sol[thetaIndex])(uDof);
      ...
      double d00 = x1[0][i]*x1[0][i] + x1[1][i]*x1[1][i];
      if(d00 < best00) { best00 = d00; dof00 = l2GMap1[i]; }
    }
    ...
    const double Dnorm = 1. + 6. / 11. * pow(eps / delta1, 2) + 3. / 143. * pow(eps / delta1, 4.);
    const double alpha = (1. / (2. * M_PI)) * M_PI * pow(delta1, 4) * Dnorm / 4.;

    std::vector<double> jacM(nDof1 * nDof1, 0.);
    ...
    for(unsigned ig = 0; ig < fem1->GetGaussPointNumber(); ig++) {
      ...
      double theta1g = 0.;
      for(unsigned i = 0; i < nDof1; i++) theta1g += solTheta1[i] * phi1[i];
      for(unsigned i = 0; i < nDof1; i++) {
        res1[i] += theta1g * phi1[i] * weight1;   // f = -theta, and RES = A u - F
      }

      // for(unsigned i = 0; i < nDof1; i++) {
      //   res1[i] -= fFrac * phi1[i] * weight1;   // added ONCE, not once per dimension
      // }

      if(tailOn) {                              // <-- INSIDE the ig loop, after GetGaussQuantities
        double solu1g = 0.;
        for(unsigned i = 0; i < nDof1; i++) solu1g += solu1[i] * phi1[i];
        for(unsigned i = 0; i < nDof1; i++) {
          res1[i] += massCoeff * solu1g * phi1[i] * weight1;
          for(unsigned j = 0; j < nDof1; j++) jacM[i * nDof1 + j] -= massCoeff * phi1[i] * phi1[j] * weight1;
        }
      }
    }                                            // <-- end of ig loop

    RES->add_vector_blocked(res1, l2GMap1);
    if(tailOn) {                       // stash; KK is not zeroed until line ~849
      tailJac.push_back(jacM);
      tailMap.push_back(l2GMap1);
    }
...
// //           res1[i] -= -2 * alpha * phi1[i] * weight1; // consistency, scaled for the fractional prefactor
...
//     RES->add_vector_blocked(res1, l2GMap1);
  ...
  if(correctConstant) {
    ...
    // double I2real = (dim == 2) ? 2. * M_PI * (delta1 - 0.75 * d0eff) : 4. / 5. * M_PI * pow(delta1, 5);   // 3D not generalized
    double I2real = (dim == 2) ? 2. * M_PI * (sqD + d0eff * d0eff / sqD - 2. * d0eff)
    : 4. / 5. * M_PI * pow(delta1, 5);   // 3D not generalized
  ...
  if(assembleFracMatrix) KK->zero(); // Set to zero all the entries of the Global Matrix

  if(tailOn && assembleFracMatrix) {
  // if(tailOn) {                         // exterior tail as a mass block, after KK is zeroed
    for(unsigned e = 0; e < tailJac.size(); e++) KK->add_matrix_blocked(tailJac[e], tailMap[e], tailMap[e]);
  }


  //BEGIN nonlocal assembly

  if(assembleFracMatrix) {
...
//                                 solu1, kappa1, delta1, printMesh);
...

  {
    ...
    Vec ones, Aones;
    MatCreateVecs(A, &ones, &Aones);
    VecSet(ones, 1.0);
    MatMult(A, ones, Aones);
    double v = 0.;
    if(dof00 >= 0) VecGetValues(Aones, 1, &dof00, &v);
    std::cout << "[ANNIHIL] (A*1) at node(0,0), dof " << dof00 << " = " << v << std::endl;
    VecDestroy(&ones); VecDestroy(&Aones);
  }

    std::cout << "RHS l1 = " << RES->l1_norm() << std::endl;   // <-- add this line
