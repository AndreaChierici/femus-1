...
int main(int argc, char** args) {
  ...
  system_specifics  system_poisson;   //me

  // =========Mesh file - BEGIN ==================
  system_poisson._mesh_files.push_back("square_-0p5-0p5x-0p5-0p5_divisions_2x2.med");
  const std::string relative_path_to_build_directory =  "../../../../";
  const std::string mesh_file = relative_path_to_build_directory + Files::mesh_folder_path() + "00_salome/2d/square/minus0p5-plus0p5_minus0p5-plus0p5/";  system_poisson._mesh_files_path_relative_to_executable.push_back(mesh_file);
 // =========Mesh file - END ==================


  system_poisson._system_name = "Biharmonic";
  system_poisson._assemble_function = NAMESPACE_FOR_BIHARMONIC_COUPLED :: poisson_equation :: AssembleBilaplaceProblem_AD;

  system_poisson._boundary_conditions_types_and_values             = SetBoundaryCondition_bc_all_dirichlet_homogeneous;


  Domains::square_m05p05::Function_Zero_on_boundary_9<> system_poisson_function_zero_on_boundary_1;

  Domains::square_m05p05::Function_Zero_on_boundary_9_Laplacian<> system_poisson_function_zero_on_boundary_1_Laplacian;



  system_poisson._assemble_function_for_rhs = &system_poisson_function_zero_on_boundary_1_Laplacian;
  system_poisson._true_solution_function = &system_poisson_function_zero_on_boundary_1;
  ...
  const std::string mesh_file_total = system_poisson._mesh_files_path_relative_to_executable[0] + "/" + system_poisson._mesh_files[0];
  ...
  for (unsigned i = 0; i < maxNumberOfMeshes; i++) {   // loop on the mesh level
    ...
    for (unsigned j = 0; j < feOrder.size(); j++) {   // loop on the FE Order
      ...
      mlSol.set_analytical_function("u", & system_poisson_function_zero_on_boundary_1);
      ...
      ml_prob.set_app_specs_pointer(& system_poisson);
      // ======= Problem, Files ========================
      ml_prob.SetFilesHandler(&files);

      // attach the boundary condition function and generate boundary data
      mlSol.AttachSetBoundaryConditionFunction( system_poisson._boundary_conditions_types_and_values );
      mlSol.GenerateBdc("u", "Steady", & ml_prob);


      // add system Biharmonic in ml_prob as a Linear Implicit System
      NonLinearImplicitSystem& system = ml_prob.add_system < NonLinearImplicitSystem > (system_poisson._system_name);

      // add solution "u" to system
      system.AddSolutionToSystemPDE("u");

      // attach the assembling function to system
      system.SetAssembleFunction( system_poisson._assemble_function );
...
// // //       // convergence for u
      std::pair< double , double > norm = GetErrorNorm_L2_H1_with_analytical_sol(& mlSol, "u",  & system_poisson_function_zero_on_boundary_1);
