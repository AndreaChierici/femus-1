...
static void AssembleBilaplaceProblem_AD(MultiLevelProblem& ml_prob) {
 ...
 const std::string solname_u = ml_sol->GetSolName_string_vec()[0];
  ...
  unsigned solFEType_sxx = ml_sol->GetSolutionType(solsxxIndex);    // get the finite element type for "v"

  unsigned solsxxPdeIndex = mlPdeSys->GetSolPdeIndex(solname_sxx.c_str());    // get the position of "v" in the pdeSys object
...
// *** get gauss point weight, test function and test function partial derivatives ***
      ...
      for (unsigned i = 0; i < nDofs; i++) {
        ...
        adept::adouble Mxyxy_s1 = 2. * phi[i] * sols1Gauss;
    ...
    if (dim == 2) {
        ...
        Bxys1 +=( (phi_x[i * dim + 1 ] * sols1Gauss_x[0] + phi_x[i * dim ] * sols1Gauss_x[1]) );
        Byys2 +=  phi_x[i * dim + 1] * sols2Gauss_x[1];
     ...
     aResu[i] += ( Bxxsxx + Bxys1 + Byys2 + F_term ) * weight;
     aRessxx[i] += (Bxxu + Mxxxx_sxx ) * weight;
     aRess1[i] += ( Bxyu + Mxyxy_s1 ) * weight;
     aRess2[i] += (Byyu + Myyyy_s2) * weight;
